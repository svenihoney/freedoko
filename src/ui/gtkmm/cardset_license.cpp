...
namespace UI_GTKMM_NS {
   ...
   ** @version   0.5.4
   **/
  CardsetLicense::CardsetLicense(Base* const parent) :
    Base(parent),
  StickyDialog("CardsetLicense", false),
  text(NULL)
  {
   ...
   **
   **/
  CardsetLicense::~CardsetLicense()
  {
    return ;
  } // CardsetLicense::~License()
   ...
   **/
  void
    CardsetLicense::license_update()
    {
      if (!this->is_realized())
	return ;

      this->text->get_buffer()->set_text(::setting(Setting::CARDSET_LICENSE));
      if (::setting(Setting::CARDSET_LICENSE).empty())
	this->text->get_buffer()->set_text("???");

      return ;
    } // void CardsetLicense::license_update()
